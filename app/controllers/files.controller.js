...
const insertNewFile = async (req, res) => {
  try {
    ...
    const currentPath = req.body.current_path;
    ...
    // const data = await filesPresenter.insertNewFile(
    //   fileName,
    //   currentPath,
    //   id_user,
    //   id_event
    ...
    let fileType = getFileType(currentPath);
    let folderPath = getFolderPath(currentPath);

    const postData = await filesRepo.insertNewFile(
      id_user,
      id_event,
      fileName,
      folderPath,
...
function getFileType(pathName) {
  ...
  switch (pathName) {
    case "/event": {
      fileType = FileTypeConstant.EVENT_BANNER;
      break;
    }
